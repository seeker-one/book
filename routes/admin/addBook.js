          ...
          db.query(`SELECT sort_name FROM sort_table`,(err,data)=>{
               if(err){
               	console.error(err);
               	  ...
               	  res.render('admin/addBook.ejs',{username:req.session['username'],type:data})
               }
          }) 
     })
     ...
     router.post('/',(req,res)=>{
          ...
          fs.rename(oldPath,newPath,(err)=>{
              if(err){
                  ...
                  db.query(`INSERT INTO book_table (book_name,book_author,sort,price,publish,publish_date,current_num,count,summary,image_src)
                    VALUES('${book_name}','${book_author}','${sort}','${price}','${publish}','${publish_date}','${count}','${count}','${summary}','${newFileName}')`,(err,data)=>{
                        if(err){
                          console.error(err);
                        }
                        else{
